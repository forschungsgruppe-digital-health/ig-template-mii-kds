...
| Decide who owns the template after 2026 | TF KDS | Currently "the MII, for now". MII funding ends end-2026 and NUM-DIZ takes over IG development and maintenance; the NUM-DIZ design is now the rendering **default** (MII stays switchable — styleguide §10) and the publisher/© line names NUM-DIZ, anticipating the handover technically — the ownership decision is separate. |
...
| Request HL7 community-use permission for the FHIR logo | [#109](../../../issues/109) | The header renders the **HL7 FHIR flame** (`fhir-logo-www.png`, shipped by `fhir2.base.template`, linked to hl7.org/fhir — mirroring kerndatensatz-basis and standard IG practice), standalone and unmodified as HL7's guidelines require, and the prescribed attribution sentence renders in every page's footer (`fragment-footer.html`, guard-tested) since the accessibility release. The one remaining step is human: file the free permission request via <https://www.hl7.org/community-use/index.cfm> (or HL7trademarks@HL7.org), naming the template and the module IGs. |
| Obtain NUM-DIZ consent for the vendored NUM-DIZ logos | [#110](../../../issues/110) | `logo-num-diz-de.svg` is the official combo fetched from the NUM website; the NUM/NUM-DIZ logos are third-party brand assets and shipping them needs NUM-DIZ (TF-KDS-level) consent — see styleguide §10. **Urgency raised:** NUM-DIZ is now the *default* design, so every rendering that does not switch to MII ships these logos out of the box — no longer an opt-in surface. |
| Obtain NUM-DIZ approval for the **derived** English combo logo | [#111](../../../issues/111) | `logo-num-diz-en.svg` is composed from two official assets **and carries English replacement text** set in a system-sans approximation (the German outlined text was translated 2026-08-14 — derivation in the file header, styleguide §10), making it doubly unofficial; as a *new* brand lockup it needs NUM-DIZ approval. Replace it the day an official English combo exists. **Urgency raised:** as part of the NUM-DIZ default it now renders on every English page of an out-of-the-box build. |
...
- **The base hardcodes the language-selector label.** `fhir2.base.template`'s
  `includes/fragment-language.html` writes the literal `Language:` instead of
  reading its own `stringsBase` catalog (which carries the translation), so
  the label never translates. This repository overrides the fragment
  verbatim-plus-one-line (catalog lookup with the English literal as
  fallback — styleguide §1); delete the override the day the base reads its
  catalog there.
