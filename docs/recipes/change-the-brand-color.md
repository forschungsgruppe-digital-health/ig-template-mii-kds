...
**Prerequisites.** A clone of this repo and the ability to build the self-test —
...
5. Rebuild the self-test (`sushi . && java -jar publisher.jar -ig ig.ini`) or push
   a `feature/*` branch and open the CI preview.
6. Open `output/index.html` (or the preview URL) and confirm the colour.

## Expected result

The rebuilt self-test IG shows your new colour on that surface, in both `/de/` and
