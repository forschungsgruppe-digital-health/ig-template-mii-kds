...
| Is the package published to a FHIR package registry? | **No.** Modules consume the template as a vendored folder (`ig-template/`, kept current automatically — see the module template's `scripts/sync-ig-template.sh`). |
