...
| `brand-switch.test.mjs` | Guards the MII/NUM-DIZ brand switch: `num-diz.css` overrides the complete MII variable set, the fragments' guards match `num-diz` exactly (unknown values degrade to MII), the documented NUM-DIZ pairs hold WCAG AA (recomputed from the shipped hex), and the derived English combo logo keeps its provenance marker | `security-scan.yml`, `dependency-check.yml` |
